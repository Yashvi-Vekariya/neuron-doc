  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const agentPromise = (async () => {
        for (let i = 0; i <= 4; i++) { setAgentStep(i); await new Promise(r => setTimeout(r, 600)); }
        setAgentStep(-1);
      })();

      const apiPromise = fetch('http://localhost:8000/api/chat/', {
        method: 'POST',
    ...
    <div className="h-full flex flex-col bg-black">
      <div className="border-b border-[#00ff41]/20 px-4 py-3 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          <button onClick={() => router.push('/dashboard')} className="text-[#1a5c26] hover:text-[#00ff41] transition-colors p-1">
            <ArrowLeft className="w-4 h-4" />
          </button>
          <div>
            <p className="font-mono text-sm text-[#00ff41]">CHAT_SESSION</p>
            <p className="text-[10px] text-[#1a5c26] font-mono">DOC_ID: {docId?.slice(0, 16)}...</p>
          </div>
        </div>
        <button onClick={() => {
          setMessages([{ id: 'welcome', role: 'assistant', content: 'Document loaded. Ask me anything about this document.' }]);
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {loading && (
          <div className="flex gap-3">
            ...
            <div className="flex-1"><AgentVisualizer currentStep={agentStep} /></div>
          </div>
        )}
      ...
      <form onSubmit={handleSubmit} className="border-t border-[#00ff41]/20 p-4 shrink-0">
        <div className="flex gap-2">
          <input ref={inputRef} type="text" value={input} onChange={e => setInput(e.target.value)}
            placeholder="Ask a question about your document..." disabled={loading} autoFocus
